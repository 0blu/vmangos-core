#ifndef NONSTD_OPTIONAL_HPP
#define NONSTD_OPTIONAL_HPP

// source: https://github.com/TartanLlama/optional/commit/3a1209de8370bf5fe16362934956144b49591565
// Custom changes:
// - rename namespace `tl::` to `nonstd::`
// - use native implementation of std::optional if >= CPP17

#ifndef   nonstd_optional_CPLUSPLUS
...
#  define nonstd_optional_CPLUSPLUS  (_MSC_VER == 1900 ? 201103L : _MSVC_LANG )
# else
#  define optional_HAVE_STD_OPTIONAL  0
#  define nonstd_optional_CPLUSPLUS  __cplusplus
# endif
#else
# define  optional_HAVE_STD_OPTIONAL  0
#endif
...
#define nonstd_CPP17_OR_GREATER  ( optional_CPLUSPLUS >= 201703L )

#define NONSTD_OPTIONAL_VERSION_MAJOR 1
#define NONSTD_OPTIONAL_VERSION_MINOR 1
#define NONSTD_OPTIONAL_VERSION_PATCH 0
...
#define NONSTD_OPTIONAL_MSVC2015
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 &&              \
     !defined(__clang__))
#define NONSTD_OPTIONAL_GCC49
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 4 &&              \
     !defined(__clang__))
#define NONSTD_OPTIONAL_GCC54
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 5 &&              \
     !defined(__clang__))
#define NONSTD_OPTIONAL_GCC55
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 &&              \
     !defined(__clang__))
// GCC < 5 doesn't support overloading on const&& for member functions
#define NONSTD_OPTIONAL_NO_CONSTRR

// GCC < 5 doesn't support some standard C++11 type traits
#define NONSTD_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)                                     \
  std::has_trivial_copy_constructor<T>::value
#define NONSTD_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T) std::has_trivial_copy_assign<T>::value

// This one will be different for GCC 5.7 if it's ever supported
#define NONSTD_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T) std::is_trivially_destructible<T>::value

// GCC 5 < v < 8 has a bug in is_trivially_copy_constructible which breaks std::vector
// for non-copyable types
#elif (defined(__GNUC__) && __GNUC__ < 8 &&                                                \
     !defined(__clang__))
#ifndef NONSTD_GCC_LESS_8_TRIVIALLY_COPY_CONSTRUCTIBLE_MUTEX
#define NONSTD_GCC_LESS_8_TRIVIALLY_COPY_CONSTRUCTIBLE_MUTEX
namespace nonstd {
  namespace detail {
      template<class T>
...
#define NONSTD_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T)                                        \
...
#define NONSTD_OPTIONAL_CXX14
#endif

// constexpr implies const in C++11, not C++14
#if (__cplusplus == 201103L || defined(NONSTD_OPTIONAL_MSVC2015) ||                \
     defined(NONSTD_OPTIONAL_GCC49))
#define NONSTD_OPTIONAL_11_CONSTEXPR
#else
#define NONSTD_OPTIONAL_11_CONSTEXPR constexpr
#endif

namespace nonstd {
#ifndef NONSTD_MONOSTATE_INPLACE_MUTEX
#define NONSTD_MONOSTATE_INPLACE_MUTEX
...
namespace detail {
#ifndef NONSTD_TRAITS_MUTEX
#define NONSTD_TRAITS_MUTEX
...
#define NONSTD_TRAITS_LIBCXX_MEM_FN_WORKAROUND
#endif

// In C++11 mode, there's an issue in libc++'s std::mem_fn
// which results in a hard-error when using it in a noexcept expression
// in some cases. This is a check to workaround the common failing case.
#ifdef NONSTD_TRAITS_LIBCXX_MEM_FN_WORKAROUND
...
struct optional_storage_base {
  NONSTD_OPTIONAL_11_CONSTEXPR optional_storage_base() noexcept
      : m_dummy(), m_has_value(false) {}

  template <class... U>
  NONSTD_OPTIONAL_11_CONSTEXPR optional_storage_base(in_place_t, U &&... u)
...
template <class T> struct optional_operations_base : optional_storage_base<T> {
  ...
  NONSTD_OPTIONAL_11_CONSTEXPR T &get() & { return this->m_value; }
  NONSTD_OPTIONAL_11_CONSTEXPR const T &get() const & { return this->m_value; }
  NONSTD_OPTIONAL_11_CONSTEXPR T &&get() && { return std::move(this->m_value); }
#ifndef NONSTD_OPTIONAL_NO_CONSTRR
...
template <class T, bool = NONSTD_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)>
...
template <class T, bool = NONSTD_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T) &&
                          NONSTD_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) &&
                          NONSTD_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)>
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
#if defined(NONSTD_OPTIONAL_CXX14) && !defined(NONSTD_OPTIONAL_GCC49) &&               \
    !defined(NONSTD_OPTIONAL_GCC54) && !defined(NONSTD_OPTIONAL_GCC55)
  /// Carries out some operation which returns an optional on the stored
  /// object if there is one.
  template <class F> NONSTD_OPTIONAL_11_CONSTEXPR auto and_then(F &&f) & {
  ...
  /// object if there is one.
  template <class F>
  NONSTD_OPTIONAL_11_CONSTEXPR detail::invoke_result_t<F, T &> and_then(F &&f) & {
    using result = detail::invoke_result_t<F, T &>;
    static_assert(detail::is_optional<result>::value,
                  "F must return an optional");
  ...
  template <class F> NONSTD_OPTIONAL_11_CONSTEXPR auto map(F &&f) & {
  ...
  /// Carries out some operation on the stored object if there is one.
  template <class F>
  NONSTD_OPTIONAL_11_CONSTEXPR decltype(optional_map_impl(std::declval<optional &>(),
                                             std::declval<F &&>()))
  map(F &&f) & {
    return optional_map_impl(*this, std::forward<F>(f));
  }
  ...
  map(F &&f) const && {
    ...
    !defined(NONSTD_OPTIONAL_GCC54) && !defined(NONSTD_OPTIONAL_GCC55)
  /// Carries out some operation on the stored object if there is one.
  template <class F> NONSTD_OPTIONAL_11_CONSTEXPR auto transform(F&& f) & {
    return optional_map_impl(*this, std::forward<F>(f));
  }
  ...
  optional<T> NONSTD_OPTIONAL_11_CONSTEXPR or_else(F &&f) & {
    if (has_value())
      return *this;
  ...
  template <class F, detail::disable_if_ret_void<F> * = nullptr>
  optional<T> NONSTD_OPTIONAL_11_CONSTEXPR or_else(F &&f) const & {
    return has_value() ? *this : std::forward<F>(f)();
  }
  ...
  NONSTD_OPTIONAL_11_CONSTEXPR optional disjunction(const optional &rhs) & {
  ...
  NONSTD_OPTIONAL_11_CONSTEXPR optional disjunction(optional &&rhs) & {
  ...
  NONSTD_OPTIONAL_11_CONSTEXPR optional(const optional &rhs) = default;

  /// Move constructor
  ///
  /// If `rhs` contains a value, the stored value is direct-initialized with
  /// it. Otherwise, the constructed optional is empty.
  NONSTD_OPTIONAL_11_CONSTEXPR optional(optional &&rhs) = default;
 ...
 template <class... Args>
  ...
  NONSTD_OPTIONAL_11_CONSTEXPR explicit optional(
  ...
  NONSTD_OPTIONAL_11_CONSTEXPR T *operator->() {
    return std::addressof(this->m_value);
  }

  /// Returns the stored value
  NONSTD_OPTIONAL_11_CONSTEXPR T &operator*() & { return this->m_value; }
  ...
  NONSTD_OPTIONAL_11_CONSTEXPR T &value() & {
    if (has_value())
      return this->m_value;
    throw bad_optional_access();
  }
  NONSTD_OPTIONAL_11_CONSTEXPR const T &value() const & {
    if (has_value())
      return this->m_value;
    throw bad_optional_access();
  }
  ...
  template <class U> NONSTD_OPTIONAL_11_CONSTEXPR T value_or(U &&u) && {
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
  ...
  /// Carries out some operation on the stored object if there is one.
  template <class F>
  NONSTD_OPTIONAL_11_CONSTEXPR decltype(detail::optional_map_impl(std::declval<optional &>(),
                                                     std::declval<F &&>()))
  map(F &&f) & {
    return detail::optional_map_impl(*this, std::forward<F>(f));
  }
  ...
  /// If `rhs` contains a value, the stored value is direct-initialized with
  /// it. Otherwise, the constructed optional is empty.
  NONSTD_OPTIONAL_11_CONSTEXPR optional(const optional &rhs) noexcept = default;
  ...
  NONSTD_OPTIONAL_11_CONSTEXPR T *operator->() noexcept { return m_value; }
  ...
  /// \group value_or
  template <class U> NONSTD_OPTIONAL_11_CONSTEXPR T value_or(U &&u) && noexcept {
    static_assert(std::is_move_constructible<T>::value &&
                      std::is_convertible<U &&, T>::value,
                  "T must be move constructible and convertible from U");
    return has_value() ? **this : static_cast<T>(std::forward<U>(u));
