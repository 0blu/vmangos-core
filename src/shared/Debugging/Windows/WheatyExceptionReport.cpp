...
#define CUSTOM_EXCEPTION_ASSERTION_FAILURE 0xC0000420L // Keep in sync with Errors.cpp
...
void WheatyExceptionReport::EnsureRegistration()
{
    // This function does nothing.
    // Registration will be done in constructor
}
...
PEXCEPTION_POINTERS pExceptionInfo)
{
    ...
    sprintf(m_szDumpFileName, "%s\\%s_%s_[%u-%02u-%02u__%02u-%02u-%02u].dmp",
        crash_folder_path, REVISION_HASH, pos, systime.wYear, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond);

    _stprintf(m_szLogFileName, _T("%s\\%s_%s_[%u-%02u-%02u_%02u-%02u-%02u].txt"),
        crash_folder_path, REVISION_HASH, pos, systime.wYear, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond);
    ...
    if (m_hDumpFile)
    {
        ...
        if (pExceptionInfo->ExceptionRecord->ExceptionCode == CUSTOM_EXCEPTION_ASSERTION_FAILURE && pExceptionInfo->ExceptionRecord->NumberParameters > 0)
        {
    ...
    if (wroteSomething)
        fprintf(stderr, "Wrote crash info to %s\n", crash_folder_path);
...
{
    wcstombs_s(nullptr, dst, src, size); // To wide to multibyte copy
}
template<size_t size>
void ToTchar(wchar_t const* src, TCHAR (&dst)[size], std::false_type)
{
    wcscpy_s(dst, src); // Plain copy
}
...
BOOL WheatyExceptionReport::_GetWindowsVersionFromWMI(TCHAR* szVersion, DWORD cntMax)
{
    ...
    if (queryResult)
    {
        do
        {
            ...
            TCHAR buf[256] = { };
            ToTchar(field.bstrVal, buf, std::is_same<TCHAR, char>::type());
...
PEXCEPTION_POINTERS pExceptionInfo)
{
    __try
    {
        ...
        if (pExceptionRecord->ExceptionCode == CUSTOM_EXCEPTION_ASSERTION_FAILURE && pExceptionRecord->NumberParameters >= 2)
        {
...
size_t countOverride)
{
        ...
        switch (basicType)
        {
            case btChar:
            {
                ...
                if (length > bufferSize - 6)
                    _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "\"%.*s...\"", static_cast<int>(bufferSize - 6), (char*)pAddress);
                else
                    _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "\"%.*s\"", static_cast<int>(length), (char*)pAddress);
                break;
            }
            case btStdString:
            {
                ...
                if (value->length() > bufferSize - 6)
                    _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "\"%.*s...\"", static_cast<int>(bufferSize - 6), value->c_str());
                else
                    _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "\"%s\"", value->c_str());
                break;
            }
            default:
                // Format appropriately (assuming it's a 1, 2, or 4 bytes (!!!)
                if (length == 1)
                    _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "0x%X", *(PBYTE)pAddress);
                else if (length == 2)
                    _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "0x%X", *(PWORD)pAddress);
                else if (length == 4)
                {
                    if (basicType == btFloat)
                        _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "%g", *(PFLOAT)pAddress);
                    else
                        _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "0x%X", *(PDWORD)pAddress);
                }
                else if (length == 8)
                {
                    if (basicType == btFloat)
                    {
                        _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "%g",
                            *(double *)pAddress);
                    }
                    else
                        _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "0x%I64X",
                        *(DWORD64*)pAddress);
                }
                else
                {
    #if _WIN64
                    _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "0x%I64X", (DWORD64)pAddress);
    #else
                    _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "0x%X", (DWORD)pAddress);
    #endif
                }
                break;
        }
    ...
    {
#if _WIN64
        _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "0x%I64X <Unable to read memory>", (DWORD64)pAddress);
#else
        _snprintf_s(pszCurrBuffer, bufferSize, _TRUNCATE, "0x%X <Unable to read memory>", (DWORD)pAddress);
#endif
    }
}
