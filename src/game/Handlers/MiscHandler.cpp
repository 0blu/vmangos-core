...
public:
    ...
    void operator()()
    {
        ...
        auto packet = std::make_unique<WorldPackets::Misc::WhoResponse>();
        packet->entries.reserve(49);
        ...
        for (const auto& itr : m)
        {
            ...
            entry.partyStatus = pPlayer->GetWhoListPartyStatus(); // not actually displayed anywhere
#endif
        ...
        uint32 count = m.size();
        packet->displayedCount = clientCount;                   // insert right count, listed count
        packet->onlineCount = count > 49 ? count : clientCount; // insert right count, online count
        sess->SendPacket(std::move(packet));
    }
};
