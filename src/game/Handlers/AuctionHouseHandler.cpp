...
{
    auto result = std::make_unique<WorldPackets::AuctionHouse::AuctionCommandResult>();
    result->auctionId = auc ? auc->Id : 0;
    ...
    switch (ErrorCode)
    {
        case AUCTION_OK:
            if (Action == AUCTION_BID_PLACED)
                result->outBid = auc->GetAuctionOutBid();
            break;
        case AUCTION_ERR_INVENTORY:
            result->inventoryError = invError;
            break;
        case AUCTION_ERR_HIGHER_BID:
            result->higherBidderGuid = ObjectGuid(HIGHGUID_PLAYER, auc->bidder);
            result->bid = auc->bid;
            result->outBid = auc->GetAuctionOutBid();
            break;
        default:
