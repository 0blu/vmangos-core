...
{
    auto sendCreateResult = [this](uint8 code)
    {
        auto response = std::make_unique<WorldPackets::Character::CharCreateResult>();
        response->code = code;
    ...
    if (GetSecurity() == SEC_PLAYER)
    {
        if (uint32 mask = sWorld.getConfig(CONFIG_UINT32_CHARACTERS_CREATING_DISABLED))
        {
            ...
            if (disabled)
            {
                sendCreateResult(CHAR_CREATE_DISABLED);
                return;
            }
        }
    }
    ...
    {
        sendCreateResult(CHAR_CREATE_FAILED);
    ...
    {
        sendCreateResult(CHAR_NAME_NO_NAME);
    ...
    if (!AllowTwoSideAccounts)
    {
        ...
        if (!characters.empty())
        {
            ...
            if (!AllowTwoSideAccounts)
            {
                ...
                {
                    sendCreateResult(CHAR_CREATE_PVP_TEAMS_VIOLATION);
                    return;
                }
            }
        }
    ...
    if (Player::SaveNewPlayer(this, guidLow, safeName, packet.race, packet.class_, packet.gender, packet.skin, packet.face, packet.hairStyle, packet.hairColor, packet.facialHair))
    {
        ...
        sendCreateResult(CHAR_CREATE_SUCCESS);
    }
    else
    {
        sendCreateResult(CHAR_CREATE_ERROR);
    }
}
...
void WorldSession::HandleCharDeleteOpcode(WorldPackets::Character::CharDelete const& packet)
{
    ...
    {
        auto response = std::make_unique<WorldPackets::Character::CharDeleteResult>();
        response->code = CHAR_DELETE_FAILED;
        SendPacket(std::move(response));
        return;
    }
...
void WorldSession::HandlePlayerLoginOpcode(WorldPackets::Character::PlayerLogin const& packet)
{
    ...
    {
        auto loginFailed = std::make_unique<WorldPackets::Character::CharacterLoginFailed>();
        loginFailed->code = 1;
        SendPacket(std::move(loginFailed));
        return;
    }
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder)
{
    ...
    auto loginVerify = std::make_unique<WorldPackets::Character::LoginVerifyWorld>();
    loginVerify->mapId = pCurrChar->GetMapId();
    ...
    {
        auto motdPacket = std::make_unique<WorldPackets::Character::GuildMotdEvent>();
        motdPacket->eventType = GE_MOTD;
        motdPacket->count = 1;
        motdPacket->motd = guild->GetMOTD();
        SendPacket(std::move(motdPacket));
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(std::unique_ptr<QueryResult> result, uint32 accountId, std::string newname)
{
    ...
    {
        auto renameResult = std::make_unique<WorldPackets::Character::CharRenameResult>();
        renameResult->code = CHAR_CREATE_ERROR;
        session->SendPacket(std::move(renameResult));
        return;
    }
    ...
    auto renameSuccess = std::make_unique<WorldPackets::Character::CharRenameResult>();
    renameSuccess->code = RESPONSE_SUCCESS;
    renameSuccess->guid = guid;
    renameSuccess->newName = newname;
    session->SendPacket(std::move(renameSuccess));
