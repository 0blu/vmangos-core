...
void Spell::EffectEnvironmentalDMG(SpellEffectIndex effIdx)
{
    ...
    if (unitTarget->GetTypeId() == TYPEID_PLAYER)
        ((Player*)unitTarget)->EnvironmentalDamage(DAMAGE_FIRE, rand_dither(damage));
    else
    {
        uint32 absorb = 0;
        int32 resist = 0;
        unitTarget->CalculateDamageAbsorbAndResist(m_caster, m_spellInfo->GetSpellSchoolMask(), SPELL_DIRECT_DAMAGE, rand_dither(damage), &absorb, &resist, m_spellInfo);
        m_caster->SendSpellNonMeleeDamageLog(unitTarget, m_spellInfo->Id, rand_dither(damage), m_spellInfo->GetSpellSchoolMask(), absorb, resist, false, 0, false);
